    ...
    >
      <div className="mx-auto hidden h-20 max-w-7xl items-center justify-between px-5 md:flex md:px-10">
        <div className="flex items-center gap-3">
          <Scissors className={`h-6 w-6 text-[#bb7c99] transition-transform duration-500 ${scrolled ? 'rotate-180' : 'rotate-90'}`} />
          <button
            type="button"
            onClick={() => {
              onGoHome();
      ...
      <div className="mx-auto flex h-20 w-full items-center justify-between px-5 md:hidden">
        ...
        >
          <Scissors className={`h-6 w-6 text-[#bb7c99] transition-transform duration-500 ${scrolled ? 'rotate-180' : 'rotate-90'}`} />
          <span className="font-sans text-base font-semibold uppercase tracking-[0.24em] text-[#4b313d]">فاطمة بوعلام</span>
        </button>
      </div>
