...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  return (
    <Link
      ...
      className="group block relative overflow-hidden bg-[#111111] aspect-[3/4] rounded-md shadow-md transition-all duration-500 ease-out"
    >
      {/* Cover Image */}
      <img
        src={project.coverImage}
        alt={project.title}
        className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-105 opacity-90 group-hover:opacity-75"
        loading="lazy"
      />
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#111111]/90 via-[#111111]/30 to-transparent flex flex-col justify-end p-6 md:p-8 z-10 transition-opacity duration-300">
        ...
        <div className="flex items-center justify-between overflow-hidden max-h-0 group-hover:max-h-12 transition-all duration-500 ease-in-out opacity-0 group-hover:opacity-100">
          ...
          <span className="text-[#c8a96a] flex items-center text-xs tracking-wider uppercase font-sans font-medium gap-1">
            Explore <ArrowUpRight className="w-3.5 h-3.5 transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-300" />
          </span>
        </div>
      </div>
    </Link>
