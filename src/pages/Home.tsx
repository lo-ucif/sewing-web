...
const PhoneIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 70 70" fill="none" aria-hidden="true">
  ...
  return (
    <div className="relative overflow-hidden pt-24 text-[#4b313d]" dir="rtl">
      <section id="home" className="px-5 pb-10 pt-2 md:px-10 lg:px-14 lg:pt-6">
        <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-8 lg:min-h-[calc(100vh-8rem)] lg:grid-cols-2 lg:gap-10">
          <div className="order-2 space-y-6 text-right lg:order-1">
            <span className="inline-flex rounded-full border border-[#f4d5e0] bg-[#fff1f7] px-4 py-2 text-sm font-medium text-[#9f6b86] shadow-sm">🧵 خياطة منزلية | الجزائر</span>
            <h1 className="text-4xl font-semibold leading-tight tracking-tight text-[#3d2734] sm:text-5xl lg:text-7xl lg:tracking-[-1.8px]">فاطمة بوعلام</h1>
            <p className="max-w-xl text-base leading-8 text-[#6b515f] sm:text-lg sm:leading-9 lg:text-xl">أصنع ملابس تقليدية وعصرية بحب واتقان من بيتي، وأحوّل كل تصميم إلى قطعة مميزة تحمل طابعك الخاص.</p>
            <div className="flex flex-wrap items-center justify-end gap-3 sm:gap-5">
              <a href="#contact" className="inline-flex h-11 items-center justify-center rounded-full border border-[#c86c94] bg-white px-6 text-sm font-semibold text-[#c86c94] transition hover:border-[#b55f83] hover:text-[#b55f83]">تواصلي معي</a>
              <button type="button" onClick={onGoGallery} className="inline-flex h-11 items-center justify-center rounded-full bg-[#c86c94] px-6 text-sm font-semibold text-white shadow-lg transition hover:bg-[#b55f83]">شوفي أعمالي</button>
            </div>
          </div>
          ...
          <div className="order-1 mx-auto flex w-full max-w-md items-center justify-center lg:order-2">
            <div className="relative h-72 w-72 sm:h-80 sm:w-80">
              ...
              <div className="relative z-10 flex h-full w-full items-center justify-center rounded-full border-8 border-white/80 bg-gradient-to-br from-[#fdeff6] via-[#fff5f8] to-[#f8e7f2] shadow-[0_25px_45px_rgba(200,120,150,0.14)]">
                <div className="flex h-[82%] w-[82%] items-center justify-center rounded-full bg-[#f9e4ef] text-5xl shadow-inner shadow-[#d8b4cf]/50 sm:text-6xl">👩🏽‍🧵</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="about" className="border-t border-[#f0d8e3] px-5 py-16 md:px-10 lg:px-14">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="rounded-[32px] border border-[#f2dbe2] bg-white/90 p-6 shadow-[0_25px_60px_rgba(224,179,200,0.12)] sm:p-8 lg:p-10">
            ...
            <div className="mt-8 text-right"><a href="#" className="inline-flex h-11 items-center rounded-full bg-[#c86c94] px-6 text-sm font-semibold text-white shadow-lg transition hover:bg-[#b55f83]">تحميل كتالوج أعمالي</a></div>
          </div>
        </div>
      </section>
      ...
      <section id="skills" className="px-5 py-16 md:px-10 lg:px-14">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 text-right"><span className="text-sm font-semibold uppercase tracking-[0.3em] text-[#b26488]">تخصصاتي</span><h2 className="mt-4 text-3xl font-semibold text-[#3d2734] sm:text-4xl lg:text-5xl">مجموعة المهارات</h2></div>
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-2">
            {skillGroups.map((group) => (
              <div key={group.title} className="rounded-[28px] border border-[#f0d8e3] bg-white/95 p-6 shadow-[0_20px_50px_rgba(216,169,194,0.12)] transition hover:-translate-y-1 hover:shadow-[0_30px_70px_rgba(216,169,194,0.18)]">
                ...
                <div className="mt-4 flex flex-wrap justify-end gap-2 sm:gap-3">{group.items.map((item) => <span key={item} className="rounded-full border border-[#f4d5e0] bg-[#fff1f7] px-3 py-2 text-xs text-[#7d5a70] shadow-sm sm:px-4 sm:text-sm">{item}</span>)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="projects" className="border-t border-[#f0d8e3] px-5 py-16 md:px-10 lg:px-14">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-10 text-right"><button type="button" onClick={onGoGallery} className="inline-flex h-11 items-center rounded-full bg-[#c86c94] px-8 text-sm font-semibold text-white shadow-lg transition hover:bg-[#b55f83]">شوفي كل الأعمال</button></div>
        </div>
      </section>

      <section id="journey" className="px-5 py-16 md:px-10 lg:px-14">
        <div className="mx-auto max-w-4xl">
          <div className="mb-10 text-right"><span className="text-sm font-semibold uppercase tracking-[0.3em] text-[#b26488]">مساري</span><h2 className="mt-4 text-3xl font-semibold text-[#3d2734] sm:text-4xl lg:text-5xl">الخبرات والتعليم</h2></div>
          <div className="relative pr-6 sm:pr-8">
            <div className="absolute bottom-0 right-2 top-0 w-[2px] bg-[#f0d0dc]" />
            <div className="space-y-8">{timelineItems.map((item) => (<div key={item.title} className="relative"><div className="absolute -right-[2px] top-2 h-4 w-4 rounded-full bg-[#c86c94] ring-4 ring-white" /><div className="rounded-[28px] border border-[#f2dbe2] bg-white p-5 shadow-[0_20px_25px_rgba(224,179,200,0.08)] sm:p-6"><h3 className="text-lg font-semibold text-[#3d2734] sm:text-xl">{item.title}</h3><p className="mt-2 text-sm text-[#6b515f]">{item.location}</p><p className="mt-3 text-sm font-semibold text-[#b26488]">{item.year}</p></div></div>))}</div>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="border-t border-[#f0d8e3] px-5 py-16 md:px-10 lg:px-14">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 text-right"><span className="text-sm font-semibold uppercase tracking-[0.3em] text-[#b26488]">تواصلي معي</span><h2 className="mt-4 text-3xl font-semibold text-[#3d2734] sm:text-4xl lg:text-5xl">هيا نبدأ تصميمك</h2></div>
          <div className="grid gap-6 md:grid-cols-2">
            <a href="tel:+212600000000" className="group rounded-[30px] border border-[#f2dbe2] bg-white p-6 text-right shadow-[0_20px_20px_rgba(220,171,197,0.1)] transition hover:-translate-y-1 hover:border-[#e7aac0] hover:bg-[#fff5f8]"><div className="flex items-center justify-between gap-4"><PhoneIcon /><div><p className="text-lg font-semibold text-[#3d2734]">الهاتف</p><p className="text-sm text-[#7d5a70]">+212 600 000 000</p></div></div></a>
            <a href="https://wa.me/212600000000" target="_blank" rel="noreferrer" className="group rounded-[30px] border border-[#f2dbe2] bg-white p-6 text-right shadow-[0_20px_20px_rgba(220,171,197,0.1)] transition hover:-translate-y-1 hover:border-[#e7aac0] hover:bg-[#fff5f8]"><div className="flex items-center justify-between gap-4"><WhatsappIcon /><div><p className="text-lg font-semibold text-[#3d2734]">واتساب</p><p className="text-sm text-[#7d5a70]">أرسل رسالة الآن</p></div></div></a>
          </div>
        </div>
      </section>
    </div>
